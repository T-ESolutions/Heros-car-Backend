...
class ModellController extends Controller
{
    ...
    {
        return view('admin.pages.modells.index');
    }

    public function create()
    {
        $brands = Brand::get();
        return view('admin.pages.modells.create',compact('brands'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.modells');
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.pages.modells.edit', compact('row','brands'));
    }
...
//            }
//        }
        $row = Modell::whereId($request->row_id)->first();
...
//            })
            ...
            ->editColumn('brand', function ($row) {
                if ($row->brand){
                    $title_ar = $row->brand->title_ar;
                    return "<b class='badge badge-dark'>$title_ar</b>";
                }else{
                    return "-";
                }
            })
...
//                if ($auth->can('sliders.update')) {
                $buttons .= '<a href="' . route('admin.modells.edit', [$row->id]) . '" class="btn btn-primary btn-circle btn-sm m-1" title="تعديل">
...
//                }
            ...
            ->rawColumns(['actions', 'image', 'active','brand'])
            ->make();
